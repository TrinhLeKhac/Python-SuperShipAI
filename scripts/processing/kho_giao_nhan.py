...
def xu_ly_kho_giao_nhan():
    ...
    print('Ninja Van')
    buu_cuc_njv_df = pd.read_excel('./input/Bưu Cục NJV.xlsx')
    buu_cuc_njv_df = buu_cuc_njv_df[1:]

    buu_cuc_njv_df.columns = [
        'region', 'receiver_province', 'id_receiver_district', 'receiver_district',
        'short_name', 'buu_cuc', 'pct_success', 'don_den_tram'
    ]
    buu_cuc_njv_df = buu_cuc_njv_df[['receiver_province', 'receiver_district', 'buu_cuc']]
    ...
    buu_cuc_njv_df['buu_cuc_short'] = buu_cuc_njv_df['buu_cuc'].astype(str).apply(split_buu_cuc_ninja_van)
    buu_cuc_njv_df['is_right'] = buu_cuc_njv_df.apply(
        lambda s: str(s['buu_cuc_short']) in str(s['receiver_district_remove_accent']), axis=1)
    ...
    buu_cuc_njv_df = buu_cuc_njv_df.groupby(['receiver_province', 'receiver_district'])['buu_cuc'].count().rename(
        'n_buu_cuc').reset_index()

    buu_cuc_njv_df.to_parquet('./processed_data/buu_cuc_ninja_van.parquet', index=False)
    print('-' * 100)

    #############################################################################################

    print('GHN')
    buu_cuc_ghn_df = pd.read_excel('./input/Bưu Cục GHN.xlsx')
    ...
    buu_cuc_ghn_df.to_parquet('./processed_data/buu_cuc_ghn.parquet', index=False)
    print('-' * 100)

    #############################################################################################

    print('BEST Express')
    buu_cuc_best_df = pd.read_excel('./input/Bưu Cục Best.xlsx')

    buu_cuc_best_df.columns = ['id', 'area', 'receiver_province', 'receiver_district', 'receiver_ward', 'buu_cuc']
    ...
    buu_cuc_best_df['buu_cuc'] = buu_cuc_best_df['buu_cuc'].str.title()

    # Chọn đúng bưu cục theo tên phường/xã
    buu_cuc_best_df['is_right'] = buu_cuc_best_df.apply(lambda s: str(s['receiver_ward']) in str(s['buu_cuc']), axis=1)
    buu_cuc_best_df = buu_cuc_best_df.loc[buu_cuc_best_df['is_right']][
        ['receiver_province', 'receiver_district', 'receiver_ward', 'buu_cuc']]
    ...
    buu_cuc_best_df = buu_cuc_best_df.groupby(['receiver_province', 'receiver_district'])['buu_cuc'].count().rename(
        'n_buu_cuc').reset_index()

    # Lưu thông tin
    buu_cuc_best_df.to_parquet('./processed_data/buu_cuc_best.parquet', index=False)
    print('-' * 100)

    #############################################################################################

    print('GHTK')
    buu_cuc_ghtk_df = pd.read_excel('./input/Bưu Cục GHTK.xlsx')

    buu_cuc_ghtk_df = buu_cuc_ghtk_df[['Tên bưu cục', 'Địa chỉ']]
    buu_cuc_ghtk_df.columns = ['buu_cuc', 'address']
    ...
    buu_cuc_ghtk_df = buu_cuc_ghtk_df.groupby(['receiver_province', 'receiver_district'])['buu_cuc'].count().rename(
        'n_buu_cuc').reset_index()

    buu_cuc_ghtk_df.to_parquet('./processed_data/buu_cuc_ghtk.parquet', index=False)
    print('-' * 100)
