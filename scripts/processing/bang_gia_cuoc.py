...
def xu_ly_bang_gia_cuoc():
    ...
    ninja_van_df['nvc'] = 'Ninja Van'
    ninja_van_df['id_nvc'] = 7
    ...
    shopee_express_df['nvc'] = 'SPX Express'
    shopee_express_df['id_nvc'] = 10
    ...
    viettel_post_df['nvc'] = 'Viettel Post'
    viettel_post_df['id_nvc'] = 4
    ...
    cuoc_phi_df = pd.concat([ninja_van_df, best_df, shopee_express_df, ghn_df, viettel_post_df, ghtk_df],
                            ignore_index=True)
    cuoc_phi_df = cuoc_phi_df[['id_nvc', 'nvc'] + COLUMNS_CUOC_PHI]
    cuoc_phi_df = cuoc_phi_df.set_index(['id_nvc', 'nvc', 'gt', 'lt_or_eq'])
    cuoc_phi_df = (cuoc_phi_df * 1000).astype(int).reset_index()

    cuoc_phi_df = pd.melt(
        cuoc_phi_df,
        id_vars=['id_nvc', 'nvc', 'gt', 'lt_or_eq'],
        value_vars=[
            'Nội Thành Tỉnh', 'Ngoại Thành Tỉnh',
            'Nội Thành Tp.HCM - HN', 'Ngoại Thành Tp.HCM - HN',
            'Nội Miền', 'Cận Miền', 'Cách Miền'
    ...
    set_nvc = set(cuoc_phi_df['nvc'].unique().tolist())
    ...
    cuoc_phi_df.to_parquet('./processed_data/cuoc_phi_nvc.parquet', index=False)
